    ...
    if isnothing(project)
        ...
        open(joinpath(dirpath, "projects.jl"), "w") do file
            ...
            for name in collect(keys(projects))
                name = titlecase(replace(name, "_kernels" => ""))
                if isfile(joinpath(dirpath, name, name * ".jl"))
                    write(file, """include("$(titlecase(name))/$(titlecase(name)).jl")\n""")
                end
            end
        end
        ...
        dirpath = abspath(
            joinpath(
                ...
                "src",
                "gen",
                titlecase(replace(project, "_kernels" => "")),
            ),
        )
        isdir(dirpath) || mkdir(dirpath)
        ...
        function sanitize(name::AbstractString) # written by ChatGPT
            sanitized = replace(name, r"[^a-zA-Z0-9]" => "_")
            if !isletter(sanitized[begin])
        ...
        open(kernelpath, "w") do file
            ...
            Threads.@threads for kernel in kernellist
                if !occursin("old_versions", kernel) && !endswith(kernel, ".pc")
                    if occursin("generic_kernels", kernel)
                        ...
                        try
                            ...
                            @error "Failed to fetch $kernel: \"$e\". This kernel will be skipped!"
                            continue
                        end
        ...
        modpath = joinpath(dirpath, titlecase(replace(project, "_kernels" => "")) * ".jl")
