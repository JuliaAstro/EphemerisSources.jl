...
"""
function code!(kernels; project = nothing)
    ...
    if isnothing(project)
        ...
        Threads.@threads for name in collect(keys(projects))
        ...
        open(joinpath(dirpath, "projects.jl"), "w") do file
            ...
            for name in collect(keys(projects))
                write(file, """include("$(lowercase(name))/$name.jl")\n""")
            end
        end
        ...
        modpath = joinpath(dirpath, lowercase(project) * ".jl")
